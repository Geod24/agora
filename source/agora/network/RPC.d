/*******************************************************************************
    ...
    It should also be fairly simple to chain generator, e.g. writting a server
                        ...
                        scope DeserializeDg dg = (size_t size)
                            {
                                ...
                                //     Ret.stringof, size);
                                if (size >= tmp.length)
                                    throw new Exception("Out of bound read");
...
// /// Generate an enum used for the dispatch of functions
// private template MethodEnum (API)
// {
//     static assert(is(API == interface), "Expected to be called with an `interface`");
...
//         {
//             static foreach (ovrld; __traits(getOverloads, API, member))
//                 result ~= format("%s = %s,\n", ovrld.mangleof, sha256Of(ovrld.mangleof));
//         }
//         return result;
//     }

//     mixin("enum MethodEnum {\n" ~ memberList() ~ `}`);
// }

// unittest
// {
//     static interface API1
//     {
//         string foo();
//         int bar(int);
//         int bar(string);
...
//     import std.traits;
//     static assert(EnumMembers!(MethodEnum!API1).length == 4);
//     static assert(__traits(identifier, EnumMembers!(MethodEnum!API1)[0]) == API1.foo.mangleof);

//     // Ensure that generation is stable, as by default static foreach
//     // will iterate from the top-level type to the low level
//     alias ME1 = MethodEnum!API1;
//     alias ME2 = MethodEnum!API2;
//     static foreach (idx, Member; EnumMembers!(ME1))
//         static assert(cast(int) Member ==
//                       cast(int) mixin("ME2.", __traits(identifier, EnumMembers!ME1[idx])));
// }
