...
public interface IBlockStorage
{
    ...
    ***************************************************************************/
    ...
    public void updateMultiSig (in Height height, in Hash hash,
...
public class BlockStorage : IBlockStorage
{
    ...
    ***************************************************************************/
    ...
    /// Implements `IBlockStorage.updateMultiSig`
    public override void updateMultiSig (in Height height, in Hash hash,
        in Signature sig, in BitField!ubyte validators) @safe
    {
        const size_t block_position = this.height_idx.findBlockPosition(height);
        if (block_position == 0)
            throw new Exception("Cannot update signatures for Genesis block");

        Block original_block = this.readBlock(height);
        if (hash != original_block.hashFull())
        ...
        static assert(BlockHeader.validators.offsetof == ValidatorsOffset,
    ...
    ***************************************************************************/
        ...
        in Signature sig, in BitField!ubyte validators) @safe
    {
        if (this.blocks.length < height)
            throw new Exception("Cannot update signature of inexistant block");
...
private void testStorage (IBlockStorage storage)
{
    ...
    storage.updateMultiSig(
        updated_block.header.height, updated_block.header.hashFull(),
